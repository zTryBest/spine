...
eq "build keeps per-task skills out of the main session under SDD" \
  "$(printf '%s' "$DECIDE4" | json_test "Array.isArray(j.rules) && j.rules.some(r=>/main session/i.test(r) && /subagent-driven-development/.test(r) && /orchestrat/i.test(r))" && echo yes || echo no)" "yes"
...
eq "new build keeps tdd/hui-pro out of the main session under SDD" \
  "$(printf '%s' "$NP_BUILD" | json_test "Array.isArray(j.rules) && j.rules.some(r=>/main session/i.test(r) && /subagent-driven-development/.test(r) && /orchestrat/i.test(r))" && echo yes || echo no)" "yes"
...
WORKFLOW_STUDIO_API="$(cd "$REPO" && "$NODE_BIN" --input-type=module -e 'const root=process.argv[1]; const { createBoardServer } = await import("./src/server.mjs"); const server=createBoardServer(root); await new Promise((resolve,reject)=>{ server.once("error", reject); server.listen(0, "127.0.0.1", resolve); }); const port=server.address().port; const html=await fetch(`http://127.0.0.1:${port}/workflows`).then(r=>r.text()); const list1=await fetch(`http://127.0.0.1:${port}/api/workflows`).then(r=>r.json()); const del=await fetch(`http://127.0.0.1:${port}/api/workflows?id=canvas-api&source=local`, { method:"DELETE" }).then(r=>r.json()); const list2=await fetch(`http://127.0.0.1:${port}/api/workflows`).then(r=>r.json()); await new Promise(resolve=>server.close(resolve)); console.log(`${html.includes("Workflow Studio")}:${list1.workflows.some(w=>w.id==="feature"&&Array.isArray(w.stages)&&w.readonly)}:${del.deleted}:${!list2.workflows.some(w=>w.id==="canvas-api"&&w.source==="local")}`);' "$T")"
eq "workflow studio page lists workflows and deletes custom workflow" "$WORKFLOW_STUDIO_API" "true:true:true:true"
