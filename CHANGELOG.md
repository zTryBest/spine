...
- **总项目看板聚焦进行中、换成总览指标(纯前端)**: 全局看板此前把已完成任务也列进"进行中的任务",还想在总览堆构建信息/工作流/阶段技能(这些是单项目的)。改为:总览的任务列表只列**进行中/待确认**(已完成/归档从列表移除,改在指标里计数);移除总览里的构建信息、工作流、工作流阶段与技能(它们只在点进单个项目后显示);新增一行**总览指标**——项目总数、活跃项目、进行中任务、待确认、待处理通知、已完成/归档;项目卡片按"待确认→进行中→其它"排序,有待确认的项目高亮。全部指标在前端从 `/api/state` 现有数据实时计算,**只改 `dashboard/index.html`,不动引擎/workflow/服务端**,刷新页面即生效、无需重跑流程。
