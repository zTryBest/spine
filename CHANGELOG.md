...
- **看板刷新降噪与详情提速**: 前端从固定 2 秒轮询改为请求完成后自适应刷新（活跃 8 秒、空闲 20 秒、页面隐藏 60 秒），避免慢请求堆叠和频繁闪屏；`ui-labels` 改为按项目缓存，不再每轮请求；项目详情下钻会立即显示加载态，并把项目列表补全改为后台加载。
- **看板状态接口瘦身**: Web `/api/state` 不再夹带技能目录深扫，技能目录只在打开 workflow 画布时通过 `/api/skills` 按需加载；CLI `board --json` 仍保留技能统计能力。

### Tests

- **看板性能回归验证**: 增加并执行针对 Web 状态接口、UI labels 项目参数、dashboard 脚本解析和 CLI workflow 自动复制的验证，确认刷新优化不影响项目级 workflow 与看板数据读取。
...
- **画布阶段规则弹窗编辑器(逐条 + 示例引导)**: 画布里阶段规则(对应 workflow YAML 的 `rules`,是多条列表)原来只有一个小 textarea,不便编辑多条。新增"展开编辑 / 查看示例"按钮,打开一个弹窗:左侧把每条规则**拆成独立一行**编辑(各带序号和删除按钮 + 底部"添加规则"),右侧一栏**可点击新增一条的示例规则**(按"技能加载 / 子代理 / 上下文探索 / 文档 / 决策"分组,中英文跟随界面语言),并有一句"什么时候 → 必须做什么"的写法引导。保存时写回该阶段 `rules`(去空白、去空行)并标记未保存。纯前端,浏览器刷新即生效。
...
- **多项目下钻项目指标置顶**: 从总看板进入单个项目详情时，顶部指标改为该项目自己的任务总数、进行中的任务、待处理通知与已完成任务；“项目总数 / 活跃项目”只在所有项目总览中展示，避免项目详情仍像跨项目统计。
- **总览任务卡片可下钻项目**: 在所有项目总览中点击“进行中的任务”卡片时，先把该任务设为所属项目 active，再自动进入对应项目详情，让任务卡和项目卡都能作为项目详情入口。
