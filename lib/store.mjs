...
export function validateWorkflow(workflow) {
  ...
  if (!Array.isArray(workflow.states) || workflow.states.length === 0) {
    die(`Workflow '${workflow.id}' has no states.`);
  }
  const ids = new Set();
  for (const s of workflow.states) {
    if (!s.id) die(`Workflow '${workflow.id}' has a state without id.`);
    if (ids.has(s.id)) die(`Workflow '${workflow.id}' has duplicate state '${s.id}'.`);
    ids.add(s.id);
  }
  const start = workflow.start || workflow.states[0].id;
  if (!ids.has(start)) die(`Workflow '${workflow.id}' start '${start}' is not a state.`);
  workflow.start = start;
  for (const s of workflow.states) {
    if (s.next && !ids.has(s.next)) die(`State '${s.id}' next '${s.next}' is not a state.`);
    if (s.fail_to && !ids.has(s.fail_to)) die(`State '${s.id}' fail_to '${s.fail_to}' is not a state.`);
    if (s.fail_when && !s.fail_to) die(`State '${s.id}' has fail_when but no fail_to.`);
    if (!s.terminal && !s.next) die(`State '${s.id}' is not terminal but has no next.`);
    if (s.needs == null) s.needs = [];
  }
}
