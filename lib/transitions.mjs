...
// be swapped, added, or reordered (via the registry / a state's `capabilities`)
// without touching the workflow graph. That is what makes free composition work.
...
export function computeNext(root, workflow, state) {
  ...
  return {
    ...
    capabilities: list(s.capabilities).map((id) => skillInfo(registry, id)),
    needs: list(s.needs),
    missing,
...
export function formatNextAction(action) {
  ...
  if (action.requiresUser) {
    lines.push('');
    lines.push('User checkpoint: stop and ask the user before recording the confirming decision.');
  }
