...
- Always run `next --json` before acting on a change. Do not infer the current state from the builtin workflow names, from memory, or from the user's last message.
...
- Before any state work, load the required capability skill through Claude Code's Skill tool, using the runtime capability returned by `next --json`, not a hardcoded builtin list. Template: **Immediately execute:** Use the Skill tool to load the <capability-id> skill. Skipping this step is prohibited. If the Skill tool is unavailable or the named skill cannot be loaded, stop and report the blocker instead of doing the state work directly.
