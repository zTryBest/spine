...
- **Workflow 编排画布**: 看板可编辑并保存项目 workflow，路径为 `.hikspine/workflows/` 或 `.hikspine/workflows/zh/`；插件内置 workflow 只作为模板来源。
...
首次运行 `workflows`、`next`、`board` 或 `ui` 时，引擎会把插件内置 workflow 模板复制到当前项目的 `.hikspine/workflows/`，只补缺失文件，不覆盖团队已经定制的 workflow。
