...
- **Workflow 编排中心**: `/workflows` 独立页面可以预览只读的内置 workflow 模板，并支持复制后保存为用户级或项目级自定义 workflow。
...
自定义 workflow 可以保存到两个可编辑 scope：

```text
用户级，保存到本机 Hikspine 目录，新项目也能加载：
...
内置 workflow 位于插件目录，是只读模板。Hikspine 不会再把内置模板自动复制到每个项目。如果同一个 workflow id 同时存在于多个 scope，Agent 必须询问用户使用哪一个，并在命令里显式传 `--workflow-source user`、`--workflow-source local` 或 `--workflow-source builtin`。
...
看板启动后，可以访问 `/workflows`，或点击顶部 **Workflow 编排** 入口进入独立编排页。内置 workflow 在这里仍然只读，需要先“复制当前”后再编辑和保存为自定义 workflow。
